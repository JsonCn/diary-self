...
var keyHandlers = {
    ...
    backspace: function ( self, event, range ) {
        ...
        //特殊处理range 敬超
        //range=self.changeRangeSround(event,range);
...
var styleToSemantic = {
    backgroundColor: {
        regexp: notWS,
        replace: function ( doc, colour,className) {
            return createElement( doc, 'SPAN', {
                'class': className?(HIGHLIGHT_CLASS+" "+className):HIGHLIGHT_CLASS,
    ...
    color: {
        ...
        replace: function ( doc, colour ,className) {
            return createElement( doc, 'SPAN', {
                'class': className?(COLOUR_CLASS+" "+className):COLOUR_CLASS,
                style: 'color:' + colour
            });
        }
    },
    ...
    fontFamily: {
        regexp: notWS,
        replace: function ( doc, family ,className) {
            return createElement( doc, 'SPAN', {
                'class': className?(FONT_FAMILY_CLASS+" "+className):FONT_FAMILY_CLASS,
    ...
    fontSize: {
        regexp: notWS,
        replace: function ( doc, size,className) {
            return createElement( doc, 'SPAN', {
                'class': className?(FONT_SIZE_CLASS+" "+className):FONT_SIZE_CLASS,
    ...
    defaults:{
        replace: function ( doc, style ,className) {
            var common={"position":true,"left":true,"top":true,"background":true}
            var css={};
            for (var i in style){
                if (common[i]&&style[i]){
                    css[i]=style[i];
            ...
            return createElement( doc, 'SPAN', {
                'class': className?("defaults"+" "+className):"defaults",
                 style: css
            });
        }
    }
...
/////敬超修改过 如果node有className,则需要保留原类名 2017/11/18
var replaceStyles = function ( node, parent ) {
    var style = node.style;
    var doc = node.ownerDocument;
    var attr, converter, css, newTreeBottom, newTreeTop, el;
    if (style.defaults){
        converter = styleToSemantic.defaults;
        el=converter.replace(doc,style,node.className);
        if ( !newTreeTop ) {
            newTreeTop = el;
        }
        if ( newTreeBottom ) {
        ...
        if( css && converter.regexp.test( css ) ) {
            el = converter.replace( doc, css,node.className);
            if ( !newTreeTop ) {
                newTreeTop = el;
            }
            if ( newTreeBottom ) {
...
function Squire ( root, config ) {
    ...
    //add BY 敬超
    this.addEventListener('mouseup',this.dealSpanItem);
    this.addEventListener('keyup',this.dealSpanItem);
...
proto._createRange =function ( range, startOffset, endContainer, endOffset ) {
...
proto.insertHTML = function ( html, isPaste ) {
    ...
    if ( typeof sanitizeToDOMFragment === 'function' ) {
        ...
        // Parse HTML into DOM tree
        ///修改人 敬超，如果html就是一个dom元素直接appendChild 2017/11/18
        div = this.createElement( 'DIV' );
...
//特殊处理有item类的span标签 敬超 2017/11/22
proto.dealSpanItem=function(event,range){
    //鼠标点击时，判断range是否是span.item
    var keyCode=event.keyCode;
    if (keyCode&&keyCode!==8&&keyCode!==37){
        return;
    }
    var range=range||this.getSelection();
    var startContainer=range.startContainer;
    var endContainer=range.endContainer;
    if (range.collapsed){
        if (endContainer.nodeType==3){
            var parent=endContainer.parentNode;
            var className=parent.className.split(" ");
            var nodeName=parent.nodeName;
            var endOffset=range.endOffset;
            if (nodeName=="SPAN"&&className.indexOf("item")>=0){
                if (!keyCode){
                  if (endOffset==endContainer.length){
                      this.changeFormat(null,{tag:"SPAN"},range);
                  }else{
...
///敬超 检查是否已经包含了某个格式 2017/11/23
proto.testPresenceinSelection=function(name,action,format,validation){
...
//敬超 检查返回action  2017/11/23
proto.formatTest=function(action){
   var temp=action;
   var test = {
    ...
    temp=temp.replace(/^([a-zA-Z])/,function(match,$1){
